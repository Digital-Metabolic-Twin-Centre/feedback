import baselineMigration from "./0001-baseline.mjs";
import createAssignedToMigration from "./0002-create-assigned-to.mjs";
import addFeedbackAssignedToMigration from "./0003-add-feedback-assigned-to.mjs";
import createNotificationSettingsMigration from "./0004-create-notification-settings.mjs";
import addNotificationPreferenceIndexesMigration from "./0005-add-notification-preference-indexes.mjs";
import addAssignedToDefaultMigration from "./0006-add-assigned-to-default.mjs";

const migrations = [
  baselineMigration,
  createAssignedToMigration,
  addFeedbackAssignedToMigration,
  createNotificationSettingsMigration,
  addNotificationPreferenceIndexesMigration,
  addAssignedToDefaultMigration,
];

function ensureMigrationsTable(db) {
  db.exec(`
    CREATE TABLE IF NOT EXISTS schema_migrations (
      id TEXT PRIMARY KEY,
      description TEXT NOT NULL,
      applied_at TEXT NOT NULL DEFAULT (strftime('%Y-%m-%dT%H:%M:%SZ', 'now'))
    )
  `);
}

function getAppliedMigrationRows(db) {
  return db
    .prepare(`SELECT id, description, applied_at FROM schema_migrations ORDER BY id ASC`)
    .all();
}

export function runSqliteMigrations(db, options = {}) {
  const logger = options.logger ?? console;

  ensureMigrationsTable(db);

  const isApplied = db.prepare(`SELECT 1 FROM schema_migrations WHERE id = ?`);

  for (const migration of migrations) {
    // Several processes (e.g. `next build` workers) may open the same file at once.
    // Take the write lock first (IMMEDIATE), then re-check, so only one of them applies it.
    db.transaction(() => {
      if (isApplied.get(migration.id)) return;

      logger?.log?.(`Applying migration ${migration.id}: ${migration.description}`);
      migration.up(db);
      db.prepare(`
        INSERT OR IGNORE INTO schema_migrations (id, description)
        VALUES (?, ?)
      `).run(migration.id, migration.description);
    }).immediate();
  }

  return getAppliedMigrationRows(db);
}

export function rollbackLastSqliteMigration(db, options = {}) {
  const logger = options.logger ?? console;

  ensureMigrationsTable(db);

  const appliedIds = new Set(getAppliedMigrationRows(db).map((row) => row.id));
  const appliedMigrations = migrations.filter((migration) => appliedIds.has(migration.id));
  const migration = appliedMigrations[appliedMigrations.length - 1];

  if (!migration) {
    logger?.log?.("No applied migrations to roll back.");
    return null;
  }

  if (typeof migration.down !== "function") {
    throw new Error(`Migration ${migration.id} does not define a down(db) rollback function.`);
  }

  logger?.log?.(`Rolling back migration ${migration.id}: ${migration.description}`);

  db.transaction(() => {
    migration.down(db);
    db.prepare(`DELETE FROM schema_migrations WHERE id = ?`).run(migration.id);
  })();

  return migration.id;
}

export function seedSqliteReferenceData(db, options = {}) {
  const logger = options.logger ?? console;

  db.transaction(() => {
    const insertType = db.prepare(`
      INSERT INTO feedback_types (name, label) VALUES (?, ?)
      ON CONFLICT(name) DO UPDATE SET label = excluded.label
    `);
    const feedbackTypes = [
      ["Bug Report", "Bug Report"],
      ["Feature Request", "Feature Request"],
      ["General Feedback", "General Feedback"],
      ["Improvement Suggestion", "Improvement Suggestion"],
      ["Data Quality Issue", "Data Quality Issue"],
      ["Other", "Other"],
    ];
    for (const [name, label] of feedbackTypes) {
      insertType.run(name, label);
    }
    logger?.log?.(`   feedback_types   -> ${feedbackTypes.length} rows`);

    const insertStatus = db.prepare(`
      INSERT INTO feedback_status (name, label) VALUES (?, ?)
      ON CONFLICT(name) DO UPDATE SET label = excluded.label
    `);
    const statuses = [
      ["Open", "Open"],
      ["In Progress", "In Progress"],
      ["Pending Review", "Pending Review"],
      ["Resolved", "Resolved"],
      ["Closed", "Closed"],
      ["Won't Fix", "Won't Fix"],
    ];
    for (const [name, label] of statuses) {
      insertStatus.run(name, label);
    }
    logger?.log?.(`   feedback_status  -> ${statuses.length} rows`);

    const insertOrg = db.prepare(`
      INSERT INTO organisations (name, label, country) VALUES (?, ?, ?)
      ON CONFLICT DO NOTHING
    `);
    const orgs = [
      ["General", "General / Other", null],
      ["Heidelberg University", "Heidelberg University", "Germany"],
      ["Erasmus MC", "Erasmus Medical Centre", "Netherlands"],
      ["Birmingham Children's", "Birmingham Children's Hospital", "United Kingdom"],
    ];
    for (const [name, label, country] of orgs) {
      insertOrg.run(name, label, country);
    }
    logger?.log?.(`   organisations    -> ${orgs.length} rows`);
  })();
}
