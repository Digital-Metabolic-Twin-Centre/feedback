...
import { z } from "zod";
import { getFeedbackById, getThreadMessages, insertThreadMessage } from "@/lib/feedback/sqlite-queries";
...
export async function POST(
  ...
  try {
    ...
    const createdBy = parsed.data.createdBy?.trim() || feedback.email || `project-key-${authResult.auth.keyId}`;
    ...
    logError(error, { operation: "v1/feedback/[id] POST", resource: req.url });
    return v1Json(
      {
        success: false,
        error: error instanceof Error ? error.message : "Internal server error",
